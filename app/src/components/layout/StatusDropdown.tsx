...
import { WorkInCliModal } from '../modals/WorkInCliModal';
...
export function StatusDropdown({ label, items, showStatus = true, theme = 'dark' }: StatusDropdownProps) {
  ...
  return (
    <>
      ...
      >
        <button 
          className={`px-3 py-1.5 text-sm flex items-center gap-2 rounded hover:bg-opacity-40 ${
            theme === 'dark'
              ? 'text-gray-300 hover:text-white hover:bg-dark-border/40'
        ...
        {isOpen && (
          <div 
            className={`absolute top-full right-0 mt-1 w-48 rounded-lg shadow-lg border py-1 z-[60] ${
              theme === 'dark'
      ...
      {showStatus && selectedItem?.status && (
        <StatusModal
          ...
          item={selectedItem as Required<StatusItem>}
        />
      )}
      ...
      <WorkInCliModal
        isOpen={showWorkInCliModal}
        onClose={() => setShowWorkInCliModal(false)}
      />
    </>
  );
}