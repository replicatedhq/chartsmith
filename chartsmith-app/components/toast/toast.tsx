...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-forge border p-6 pr-8 shadow-lg backdrop-blur-sm transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border-forge-iron bg-forge-charcoal/95 text-stone-100",
        destructive: "border-red-500/50 bg-red-500/10 text-red-50",
        success: "border-emerald-500/50 bg-emerald-500/10 text-emerald-50",
        warning: "border-forge-ember/50 bg-forge-ember/10 text-forge-ember-bright",
      },
    },
    defaultVariants: {
      variant: "default",
...
const ToastClose = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Close>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Close>>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-forge p-1 text-forge-zinc opacity-0 transition-all hover:text-stone-100 hover:bg-forge-iron/50 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-forge-ember/50 group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400",
      className,
    )}
    toast-close=""
    {...props}
