...
const importExportItems = [{ label: "Download" }, { label: "Work in CLI" }, { label: "Publish to Replicated" }];
      ...
      <nav className={`h-14 border-b flex items-center justify-between px-4 relative z-50 ${theme === "dark" ? "border-dark-border bg-dark-surface" : "border-gray-200 bg-white"}`}>
        <Link href="/" className={`flex items-center space-x-2 min-w-[200px] ${theme === "dark" ? "text-white" : "text-gray-900"}`}>
          ...
          <div className="flex items-center">
            <span className={`text-lg font-semibold ${theme === "dark" ? "text-gray-300" : "text-gray-900"} ml-2`}>ChartSmith</span>
            <span className={`text-xs ${theme === "dark" ? "text-gray-400" : "text-gray-500"} ml-2`}>by Replicated</span>
          </div>
        </Link>

        <div className="flex-1 flex justify-center">
          <div className="relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} ref={dropdownRef}>
            <button className={`px-3 py-1.5 text-sm flex items-center gap-2 rounded hover:bg-opacity-40 ${theme === "dark" ? "text-gray-300 hover:text-white hover:bg-dark-border" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}>
              my-helm-chart
              <ChevronDown className="w-4 h-4" />
            </button>

            {isChartDropdownOpen && <div className={`absolute top-full left-1/2 -translate-x-1/2 mt-1 w-64 rounded-lg shadow-lg border py-1 z-[60] ${theme === "dark" ? "bg-dark-surface border-dark-border" : "bg-white border-gray-200"}`}>{/* ... dropdown content ... */}</div>}
          </div>
        </div>
